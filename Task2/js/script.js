...
let skills = _.map(json, 'skills');
skills = _.flatten(skills);
skills = _.uniq(skills);
skills = _.orderBy(skills, [skill => skill.toLowerCase()], ['asc']);

console.log(skills);


let sortedByFriends = _.sortBy(json, ['friends.length']);
let names = _.map(sortedByFriends, 'name');

console.log(names);

let friendsAll = [];
let friendsArr = _.map(json, 'friends');
let friendsArrNames = _.map(friendsArr, function(innerArr) {
    return _.map(innerArr, 'name');
});
j = _.map(friendsArrNames, function(arr) {

    friendsAll = friendsAll.concat(arr);

});

friendsAll = _.uniq(friendsAll);
